...
# docker build -t russ_live .
# docker run -d -p 80:80 -e RAILS_MASTER_KEY=<value from config/master.key> --name russ_live russ_live
...
# Start server via nginx + Puma by default, this can be overwritten at runtime
EXPOSE 80
CMD ["./bin/production-server"]
